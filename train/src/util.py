...
def full_contrastive_scores_and_labels(query: torch.Tensor,
    ...
    labels = torch.zeros(batchsize, dtype=torch.long).to(key.device)
    # print(query.shape, key.shape, key.t().shape)
    query = query.repeat_interleave(train_n_passages, dim=0)
    # print(query.shape, key.shape)
    scores = torch.sum(query * key, dim=1) # dot product
    scores = scores.view(batchsize, -1) # [batchsize, train_n_passages]
    return scores, labels

def full_contrastive_scores_and_labels_with_inbatch(query: torch.Tensor,
                                       key: torch.Tensor,
                                       use_all_pairs: bool = True) -> Tuple[torch.Tensor, torch.Tensor]:
    assert key.shape[0] % query.shape[0] == 0, '{} % {} > 0'.format(key.shape[0], query.shape[0])
    # print(f'query.shape:{query.shape}')  [bs*ngpu, 748]
    # print(f'key.shape:{key.shape}')   [bs*ngpu*n_psg, 748]
    train_n_passages = key.shape[0] // query.shape[0]
